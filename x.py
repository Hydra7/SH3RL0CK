...
def banner():
    print("""%s
                  ...
                  |____/|_| |_|____/|_| \_\_____\___/ \____|_|\_\%s%s

                        -= Property of Dark-IT Security =-
    """ % (R, W, Y))


def parser_error(errmsg):
    banner()
    print("Command: python " + sys.argv[0] + " [Options] use -h for help")
    sys.exit()
...
class enumratorBase(object):
    ...
    def print_banner(self):
        ...
        self.print_(G + "[+] Looking for %s.." % (self.engine_name) + W)
        return
...
class GoogleEnum(enumratorBaseThreaded):
    ...
    def check_response_errors(self, resp):
        if 'Our systems have detected unusual traffic' in resp:
            self.print_(R + "[!] WARNING!!! Google probably now is blocking our requests" + W)
            self.print_(R + "[+] Stay Calm & Please wait..." + W)
            return False
        return True
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if not domain_check.match(domain):
        if not silent:
            print(R + "-_- Please enter a valid domain" + W)
        return []
    ...
    if not silent:
        print(B + "[+] Searching All Subdomain from %s" % parsed_domain.netloc + W)
    ...
    if subdomains:
        ...
        if not silent:
            print(Y + "[+] Total Subdomains Found: %s" % len(subdomains) + W)
